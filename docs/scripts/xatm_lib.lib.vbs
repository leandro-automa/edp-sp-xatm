...
Sub ShowReport()
	...
	' What is installed, written out and put in front of somebody.
	'
	...
	' Said as unknown and never guessed at. Nothing orders the start of one
	' object against another, so a report asked for early can find that no
	' device has written its build flag yet.
	Dim edition
	edition = LibraryEdition()
	If edition = "" Then edition = "unknown - no device has reported its build"

	info.Add "Library",       "xatm_lib"
	info.Add "Version",       LIBRARY_VERSION
	info.Add "Edition",       edition
	info.Add "Generated",     Now
	info.Add "Release notes", RELEASE_NOTES

	' No dialogue on the way out, either way. This runs in a data server,
	' where a MsgBox is a window nobody may be standing in front of and the
	' domain waits on it regardless. What went wrong goes to the console,
	' which is where the rest of the library says things.
	Dim path
	path = WriteReport(info)
...
' Renders the dictionary and writes it to a file, returning where it went -
' "" when it went nowhere.
'
' The temporary folder rather than beside the project: the project's folder
' may be read only on a running station, and a report nobody can write is
' worse than one that lands somewhere ordinary.
Function WriteReport(info)
	...
	If fso Is Nothing Then
		WriteLog "There is no file system object to write the report with."
		Exit Function
	End If
	...
	On Error Resume Next
	folder = fso.GetSpecialFolder(TEMPORARY_FOLDER).Path
	On Error Goto 0

	If Trim(folder & "") = "" Then
		WriteLog "The temporary folder could not be read, so no report was written."
		Exit Function
	End If
	...
	If failed <> "" Then
		WriteLog "The report would not write to " & path & " - " & failed
		Exit Function
	End If
...
' Hands the file to whatever the machine opens a .txt with. Nothing is
' assumed about which editor that is.
'
' It opens on the machine running the data server, which on a station with
' remote viewers is not the machine somebody clicked on. The console line
' names the path either way, so there is always somewhere to go and look.
Sub OpenFile(path)
	...
	If shell Is Nothing Then
		WriteLog "The report is at " & path & " - there is no shell to open it with."
		Exit Sub
	End If
	...
	If failed <> "" Then
		WriteLog "The report is at " & path & " - it would not open: " & failed
	Else
		WriteLog "Version report written to " & path & "."
	End If

End Sub


' Scripting's own number for the temporary folder, and what the report is
' called once it is in there.
